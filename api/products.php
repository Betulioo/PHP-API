...
switch ($method) {
    ...
    case 'DELETE':
        if (isset($_GET['id'])) {
            ...
            // Primero eliminamos todos los registros en product_invoice relacionados con este producto
            $deleteProductInvoicesSql = "DELETE FROM product_invoice WHERE product_id = $id";
            if ($conn->query($deleteProductInvoicesSql) === TRUE) {
                ...
                if ($result === TRUE) {
                    ...
                    if ($conn->affected_rows == 1) {
                        ...
                        // si no es 1, regreso un mensaje de error diciendo que no se encontro el producto
                ...
                // si hay un error en la eliminacion de los registros de product_invoice, regreso un error
                $response = ["METHOD" => "DELETE", "SUCCESS" => false, "ERROR" => $conn->error];
            }
        } else {
